...
class Config {
  constructor (options) {
    ...
    const protocol = coalesce(options.protocol, 'http')
    ...
    const path = coalesce(options.path, '/v0.3/traces')
    ...
    this.url = new URL(`${protocol}://${hostname}${port === '' ? '' : `:${port}`}${path}`)
    this.params = coalesce(options.params, {});
